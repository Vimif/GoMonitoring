...
func ServiceAction(client *ssh.Client, serviceName, action, osType string) error {
	...
	// SÉCURITÉ: Valider le nom du service pour éviter l'injection de commandes
	...
	if osType == "windows" {
		// PowerShell commands
		// Note: Nécessite des droits admin. On suppose que l'utilisateur SSH a les droits.
		psAction := action
		if action == "restart" {
			psAction = "Restart"
		} else if action == "stop" {
		...
		// Linux (Systemd)
		// Note: Nécessite souvent sudo.
		// On tente avec sudo -n (non-interactive) pour voir si ça passe sans mot de passe
		// Sinon la commande échouera et l'erreur sera remontée
		cmd = fmt.Sprintf("sudo -n systemctl %s %s", action, serviceName)
	}

	output, err := client.Execute(cmd)
	if err != nil {
		return fmt.Errorf("erreur exécution '%s': %v (Output: %s)", cmd, err, strings.TrimSpace(string(output)))
	}
